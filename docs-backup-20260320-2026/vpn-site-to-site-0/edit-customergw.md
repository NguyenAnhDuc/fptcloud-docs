---
sidebar_class_name: hidden
sidebar_label: "Hướng dẫn chỉnh sửa Customer gateway"
title: "Hướng dẫn chỉnh sửa Customer gateway"
source: "https://fptcloud.com/documents/vpn-site-to-site-0/?doc=edit-customergw"
parent: "https://fptcloud.com/documents/vpn-site-to-site-0"
lang: vi
converted: "2026-03-19"
---

# Hướng dẫn chỉnh sửa Customer gateway


* * *

**_Lưu ý: Chỉ chỉnh sửa được Customer gateway khi CGW không được gán vào VPN connection_**

Để chỉnh sửa **Customer gateway** , người dùng thao tác theo hướng dẫn sau:

**Bước 1:** Tại màn hiển thị danh sách **Customer gateway,** nhấn **Edit Customer gateway** tại mục **Action** của **Customer gateway** cần chỉnh sửa. ![file](/img/docs/c8f82034a89e-image-1744269804714.png)

**Bước 2:** Tại đây, người dùng chỉnh sửa các thông tin bao gồm: ![file](/img/docs/b090793a27c5-image-1744269819671.png)

**Trường** | **Mô tả**  
---|---  
**Customer gateway name** | Tên của Customer gateway  
**Description** | Mô tả Customer gateway  
**Remote IP public**   
  
| Địa chỉ IP public của VPN Connection phía On-premise  
**Remote network** | Subnet phía On-premise  
**Healthcheck IP address** | Địa chỉ IP được sử dụng để kiểm tra tình trạng hoạt động của kết nối VPN giữa hai địa điểm  
  
  
**Bước 3:** Nhấn **Update Customer gateway** để cập nhật hoặc **Cancel** để huỷ bỏ ![file](/img/docs/ef24471ffabe-image-1744269832390.png)

[ Previous Hướng dẫn tạo Customer gateway ](./create-customergw) [ Next Hướng dẫn xoá Customer gateway ](./delete-customergw)
