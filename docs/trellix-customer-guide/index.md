---
sidebar_label: "Trellix Customer Guide"
title: "2. Trellix Customer Guide"
source: "https://fptcloud.com/documents/trellix-customer-guide/"
lang: vi
converted: "2026-03-19"
---

# 2. Trellix Customer Guide


Trellix Customer Guide



##  Giới thiệu 

Hướng dẫn khách hàng về dịch vụ Anti-Virus của FPT Smart Cloud bao gồm các thao tác đăng nhập, cài đặt vận hành hệ thống quản trị Anti-Virus.

## Hướng dẫn chi tiết 

  * [ Truy cập giao diện quản trị ](./truy-cap-giao-dien-quan-tri)
  * [ Tạo gói cài đặt Agent, cài đặt Agert ](./setting-agert)
  * [ Quản lý các tác vụ chạy trên máy chủ (Client Tasks) ](./client-task)
  * [ Báo cáo (Report) ](./report)
  * [ Xem nhật ký (Log) ](./view-log)

