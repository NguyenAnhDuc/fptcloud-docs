---
sidebar_class_name: hidden
sidebar_label: "Huong dan nang goi dich vu dang su dung"
title: "2. FPT Container Registry"
source: "https://fptcloud.com/documents/fpt-container-registry-2/?doc=huong-dan-nang-goi-dich-vu-dang-su-dung"
parent: "https://fptcloud.com/documents/fpt-container-registry-2"
lang: vi
converted: "2026-03-19"
---

# 2. FPT Container Registry


Hướng dẫn nâng gói dịch vụ đang sử dụng 


* * *

FPT Cloud hỗ trợ người dùng nâng gói dịch vụ từ gói thấp hơn lên gói cao hơn thực hiện như sau: 

**Bước 1** : Tại menu **FPT Portal** > Chọn **Container Registry** > **Configuration** > **Click Update Type** để thực hiện nâng gói 

![](/img/docs/62c0048a6526-Picture86.png)

**Bước 2** : Lựa chọn gói dịch vụ muốn nâng 

![](/img/docs/793fd481cf22-Picture87.png)

**Bước 3** : Click **Update** để thực hiện nâng gói 

![](/img/docs/0cf71a94c0f2-Picture88.png)

**Bước 4** : Kết quả sau khi nâng gói dịch vụ: 

![](/img/docs/ca8ca84b98de-Picture89.png)

[ Previous Xem chi tiết Quota dịch vụ ](./cem-chi-tiet-quota-dich-vu) [ Next Scan Images  ](./scan-image)
