---
sidebar_label: "Quản lý Repositories trên FPT Portal"
title: "2. FPT Container Registry"
source: "https://fptcloud.com/documents/fpt-container-registry-2/?doc=quan-ly-repositories-tren-fpt-portal"
parent: "https://fptcloud.com/documents/fpt-container-registry-2"
lang: vi
converted: "2026-03-19"
---

# 2. FPT Container Registry


Quản lý Repositories trên FPT Portal


* * *

[ Previous Pull docker image từ FPT Container Registry về local ](./pull-docker-image-tu-fpt-container-registry-ve-local) [ Next Xem danh sách Repositories ](./xem-danh-sach-repositories)
