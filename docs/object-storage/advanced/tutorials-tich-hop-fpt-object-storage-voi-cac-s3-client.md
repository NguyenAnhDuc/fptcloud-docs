...
Trước khi tích hợp, bạn cần thu thập đủ thông tin xác thực như Endpoint và Access Key từ FPT Object Storage theo hướng dẫn tại phần **[Quản lý Access Key](https://fptcloud.com/documents/object-storage/?doc=tutorials-quan-ly-access-key-cua-object-storage-tren-fpt-portal)**.
...
**Bước 2:** Nhấp vào nút **Open Connection** trên giao diện chính hoặc chọn **File** -> **Open Connection** từ menu. ![file](/img/docs/ef4d33a6f960-image-1692698326176.png) **Bước 3:** Một hộp thoại **Open Connection** sẽ xuất hiện. Trong hộp thoại này, nhập các thông tin xác thực như sau:
  ...
  * **Secret Key ID:** Secret Key lấy từ FPT Portal. ![file](/img/docs/bab6ef92ccbb-image-1692698347523.png) Sau khi đã nhập thông tin xác thực, nhấn **Connect** để kết nối.

Cyberduck sẽ thực hiện kết nối với FPT Object Storage sử dụng các thông tin xác thực bạn đã cung cấp. Sau khi kết nối thành công, bạn sẽ thấy cấu trúc thư mục trên FPT Object Storage xuất hiện trong cửa sổ Cyberduck. ![file](/img/docs/9866d9f68092-image-1692698365189.png) Từ đây, bạn có thể thực hiện các thao tác tải lên, tải xuống và quản lý tệp trực tiếp thông qua giao diện trực quan của Cyberduck.
...
**Bước 2.** Nhấp vào **New Session** để bắt đầu thiết lập một phiên kết nối mới. ![file](/img/docs/52896479ba5d-image-1692698404134.png) **Bước 3.** Trong cửa sổ **Login** , thực hiện các bước sau:

  * **File protocol:** Chọn **"Amazon S3"**
  * **Access Key ID:** Access Key lấy từ FPT Portal.
  * **Secret Access Key:** Secret Key lấy từ FPT Portal.
  * **Host name:** Endpoint lấy từ FPT Portal. Lưu ý bỏ phần https, ví dụ: s3-sgn09.fptcloud.com.
  * **Port number:** 443\. ![file](/img/docs/1e029ed5f0c4-image-1692698415054.png) **Bước 4.** Nhấn **Save** để lưu cấu hình và **Login** để thực hiện kết nối.
...
Upload dữ liệu từ local lên FPT Object Storage:
    ...
    ./s3sync --tk  --ts  --te "target endpoint" -w 64 fs:// s3://  --sync-progress --s3-acl public-read
...
Đồng bộ dữ liệu từ Amazon S3 về FPT Object Storage
    ...
    ./s3sync --sk  --ss  --tk  --ts  --te "target endpoint" -w 64 s3:// s3:// --sync-progress --s3-acl public-read --disable-http2

[ Previous Sử dụng Object Storage trên S3 Browser ](./tutorials-su-dung-object-storage-tren-s3-client) [ Next SDK API Reference ](./sdk-api-reference)
